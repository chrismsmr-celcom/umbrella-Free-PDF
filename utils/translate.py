...
# Configuration avec plusieurs URLs de fallback
LIBRETRANSLATE_URLS = [
...
async def handle_translation(input_path: str, temp_dir: str, target_lang: str, layout: str = "text") -> Optional[str]:
    """
    Gere la traduction d'un PDF avec fallback entre plusieurs API gratuites
    """
    ...
    try:
        ...
        if not translated_pages:
            print("Echec de la traduction")
            return None
        
        print(f"Generation du PDF traduit...")
        
        result = await generate_pdf_text_only(translated_pages, output_path)
        
        if result and os.path.exists(output_path):
            print(f"PDF traduit genere: {os.path.basename(output_path)}")
            return output_path
...
async def translate_pdf_content(pages_content: List[dict], target_lang: str) -> List[dict]:
    """
    Traduit le contenu du PDF page par page
    """
    translated_pages = []
...
def split_text_into_chunks(text: str, max_length: int) -> List[str]:
    """
    Decoupe le texte en chunks pour l'API
    """
    chunks = []
...
async def translate_text_with_fallback(text: str, target_lang: str) -> str:
    ...
    try:
        ...
        print(f"Lingva echoue: {e}")
    
    # Essayer LibreTranslate sur plusieurs URLs
    for url in LIBRETRANSLATE_URLS:
        try:
            result = await libretranslate_request(text, target_lang, url)
            if result != text:
                return result
        except Exception as e:
            print(f"LibreTranslate ({url}) echoue: {e}")
    ...
    try:
        ...
        print(f"MyMemory echoue: {e}")
    
    print(f"Aucune API disponible, texte non traduit")
    return text

async def lingva_translate(text: str, target_lang: str) -> str:
    """
    Utilise l'API Lingva
    """
    try:
        source_lang = "auto"
        encoded_text = urllib.parse.quote(text[:2000])
        ...
        print(f"Erreur Lingva: {e}")
        return text

async def libretranslate_request(text: str, target_lang: str, url: str) -> str:
    """
    Utilise l'API LibreTranslate
    """
    try:
...
async def generate_pdf_text_only(translated_pages: List[dict], output_path: str) -> bool:
    """
    Genere un PDF simple avec le texte traduit
    """
    try:
        ...
        for page_info in translated_pages:
            ...
            for line in lines:
                ...
                c.drawString(margin, y_position, line[:200])
        ...
        print(f"Erreur generation PDF texte: {e}")
        return False

def is_language_supported(lang: str) -> bool:
    """
    Verifie si la langue est supportee par les API
    """
...
async def batch_translate(pdf_paths: List[str], output_dir: str, target_lang: str) -> List[str]:
    """
    Traduit plusieurs PDFs en sequentiel
    """
    results = []
